...
//    this.parentWindow = parentWindow;
    try {
      ...
      num++;
      addButtonToContainer(num, "changeQuotes", "WTChangeQuotesSmall.png", "loMenuChangeQuotes", buttonContainer);
      num++;
...
//    contentWindow.setPosSize(rect.X, rect.Y, rect.Width, rect.Height, PosSize.POSSIZE);
  ...
  private WtProofreadingError[] getErrorsOfParagraph(TextParagraph tPara, WtSingleDocument document) {
    ...
    return document.mergeErrors(errors, nFPara, false);
  }
  ...
  private String reconstructAiTextFromCache(TextParagraph tPara, WtSingleDocument document) throws Throwable {
    ...
    if(pErrors != null) {
      for (int i = pErrors.length - 1; i >= 0; i--) {
        ...
        if (error.nErrorStart <= sPara.length() && error.aSuggestions.length > 0) {
...
*/
  ...
  public static Color getCacheStatusColor(int size) {
    if (size < 50) {
      ...
      return new Color((int) (200 - (size - 50) * 4.), 200 - (int) ((size - 50) * 1.6), 0);
    }
  }
