 ...
 * @author Fred Kruse
 */
public class WtViewCursorTools {
  
  private static int isBusy = 0;

  private XComponent xComponent;

  public WtViewCursorTools(XComponent xComponent) {
  ...
  public String getAndSelectViewCursorParagraph() {
    try {
      ...
      if (xVCursor == null) {
        WtMessageHandler.printToLogFile("xVCursor == null");
        return null;
      }
      XText xVCursorText = xVCursor.getText();
      if (xVCursorText == null) {
        WtMessageHandler.printToLogFile("xVCursorText == null");
        return null;
      }
...
/*
  public void insertText(String text, boolean override) {
    ...
    try {
      if (text != null && xComponent != null) {
        ...
        XTextViewCursor xVCursor = getViewCursor();
        if (xVCursor == null) {
          WtMessageHandler.printToLogFile("ViewCursorTool: insertText: xVCursor == null");
          return;
        }
  ...
  private static int setViewCursorToParaIfFits(int xChar, int numPara, int nLastPara, XText xText, XTextViewCursor vCursor) {
    ...
    if (xParagraphCursor != null) {
      ...
      WtMessageHandler.printToLogFile("ViewCursorTools: setViewCursorToParaIfFits: xParagraphCursor == null");
    }
    return nLastPara;
  }
  ...
  public XIndexAccess getIndexAccessOfAllTables() {
    ...
    try {
      ...
      return xTableSupplier == null ? null : UnoRuntime.queryInterface(XIndexAccess.class, xTableSupplier.getTextTables());
    } catch (Throwable t) {
      WtMessageHandler.printException(t);     // all Exceptions XWordCursorthrown by UnoRuntime.queryInterface are caught
      return null;           // Return null as method failed
    } finally {
  ...
  public void setTextViewCursor(int xChar, TextParagraph yPara)  {
    isBusy++;
    try {
      if (yPara.type == WtDocumentCache.CURSOR_TYPE_TEXT) {
        setDocumentTextViewCursor(xChar, yPara.number);
      } else if (yPara.type == WtDocumentCache.CURSOR_TYPE_TABLE) {
        setViewCursorToParagraphOfTable(xChar, yPara.number);
      } else if (yPara.type == WtDocumentCache.CURSOR_TYPE_SHAPE) {
        setViewCursorToParagraphOfShape(xChar, yPara.number);
      } else if (yPara.type == WtDocumentCache.CURSOR_TYPE_FOOTNOTE) {
        setViewCursorToParagraphOfFootnote(xChar, yPara.number);
      } else if (yPara.type == WtDocumentCache.CURSOR_TYPE_ENDNOTE) {
        setViewCursorToParagraphOfEndnote(xChar, yPara.number);
      } else if (yPara.type == WtDocumentCache.CURSOR_TYPE_HEADER_FOOTER) {
